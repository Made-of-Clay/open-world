...
2. **Islands**
   ...
   - Half-sphere `SphereGeometry` scaled flat, 3-4 islands scattered, different radii. Ground/none at origin.
   - Collision per island: height map = f(dist from island center).
3. **Sky + clouds + daylight**
...
4. **Fence**
   - Invisible circular (or box) world boundary; clamp avatar position.
